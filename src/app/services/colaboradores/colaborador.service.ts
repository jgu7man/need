...
export class ColaboradorService {
    ...
    async getCoPerfil() {
        ...
        this._auth.colab$.pipe().subscribe( async (colab: ColaboradorModel) => {
        ...
        if ( colab ) {
            ...
            this.coPerfil.next( colab )

            console.log(colab);
            if (colab.estado == 'solicitud') {
                if ( !colab.nombre || !colab.apellido_materno || !colab.apellido_paterno ) {
                    return this.router.navigate( [ '/colaborador/registro' ] ),
                        $( 'app-loading' ).fadeOut();
                }
                var datosDoc = await coRef.collection( 'info' ).doc( 'datos' ).get()
                if ( !datosDoc.exists ) {
                    return this.router.navigate( [ '/colaborador/reg-datos', colab.uid ] ),
                        $( 'app-loading' ).fadeOut();
                }

                var expLaboralDoc = await coRef.collection( 'info' ).doc( 'exp_laboral' ).get()
                if ( !expLaboralDoc.exists ) {
                    return this.router.navigate( [ '/colaborador/exp_laboral', colab.uid ] ),
                        $( 'app-loading' ).fadeOut();
                }

                if ( !colab.imgPerfil || !colab.identFront || !colab.identBack ) {
